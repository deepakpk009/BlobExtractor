 ...
 */
package com.deepak.blobextractor.arrayprocessing.optimized.level1;
...
public class BlobExtractor {
    ...
    public ArrayList<BufferedImage> extractBlobs(BufferedImage sourceImage) {
        ...
        if (isValidImage(sourceImage)) {
            // take the copy of the original image as int array
    ...
    private Rectangle getBlobSize(int blobLabel) {
        ...
        // find x1
        // scanning column wise from left to right of the image
        for (int i = 0; i < w; i++) {
            for (int j = 0; j < h; j++) {
        ...
        // find y1
        // scanning row wise from left to right of the image
        for (int j = 0; j < h; j++) {
            for (int i = 0; i < w; i++) {
                // on fining a pixel with the specified label
                if (img[i + (j * w)] == blobLabel) {
        ...
        // find x2
        // scanning column wise from right to left of the image
        for (int i = w - 1; i > 0; i--) {
            for (int j = 0; j < h; j++) {
                // on fining a pixel with the specified label
                if (img[i + (j * w)] == blobLabel) {
        ...
        // find y2
        // scanning row wise from right to left of the image
        for (int j = h - 1; j > 0; j--) {
            for (int i = 0; i < w; i++) {
                // on fining a pixel with the specified label
                if (img[i + (j * w)] == blobLabel) {
    ...
    private BufferedImage getBlob(int blobLabel) {
        ...
        // blob image array
        int b[] = new int[blobSize.width * blobSize.height];
        ...
        for (int j = blobSize.y, y = 0; j < (blobSize.y + blobSize.height - 1); j++, y++) {
            for (int i = blobSize.x, x = 0; i < (blobSize.x + blobSize.width - 1); i++, x++) {
                ...
                if (img[i + (j * w)] == blobLabel) {
                    ...
                    b[x + (y * blobSize.width)] = BLOBCOLOR;
                    // increment blob pixel counter
                    blobPixelCount++;
                } else {
                    b[x + (y * blobSize.width)] = BACKGROUNDCOLOR;
                }
            }
        }
        ...
        blob.setRGB(0, 0, blobSize.width, blobSize.height, b, 0, blobSize.width);
    ...
    private boolean isAllKernelPixelsSame(int x, int y) {
        ...
        return (sumOfAllKernelPixelValues / 9 == img[x + (y * w)]);
    }
